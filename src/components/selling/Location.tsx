  ...
  async function fetchAddressFromLatLong(lat: string, lng: string) {
    try {
      ...
      if (data.status === "OK") {
        ...
        const shortAddress = `${addressComponents[2].short_name}, ${
  ...
  return (
    <>
      <div className="p-2 sm:p-0 w-full ">
        <div className="grid w-full max-w-sm m-auto sm:max-w-md gap-1.5">
          <Label htmlFor="zip-code">Zip Code</Label>
          <div className="overflow-hidden flex items-center gap-2 border border-gray rounded-3xl p-1 sm:p-2 ">
          <Input
            type="text"
            id="zip-code"
          ...
          />
          <div className="sm:px-4 px-2  inline-flex items-center justify-center">
      ...
      {zipCodeError ? <p className="text-xs sm:text-sm text-red-500 mt-2">{zipCodeError}</p> : ""}
      {/* <p className="text-sm text-red-500 mt-2">{zipCodeError}</p> */}

      <p className="text-xl text-center text-gray-200">OR</p>
      <div className="flex flex-col sm:flex-row w-full p-2 sm:p-0 max-w-md gap-1.5">
        <Button
          variant="outline"
